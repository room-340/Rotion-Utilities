...
%     skipper = 1;
    ...
    pVec = basePosition + baseVector;
    pic = line([basePosition(1) pVec(1)],[basePosition(2) pVec(2)],[basePosition(3) pVec(3)], 'LineWidth', 2, 'Color', 'k');
...
%         pause(0.01);
        delete(pic);
%         delete(ptr);
        vector = quatrotate(conjq(quaternion(k,:)), baseVector);
%         pVec = quatrotate(quaternion(k,:), pointerVector);
%         pVec = vector + pVec;
        vector = basePosition + vector;
